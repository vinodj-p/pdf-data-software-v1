...
public class DocumentProcessor {
	...
	private static void processToTextract(String strBucket, String strDocument, ProcessType eType) throws Exception {
        ...
        {
            case DETECTION:
                StartDocumentTextDetection(strBucket, strDocument);
                if(isNullOrEmpty(startJobId)) {
                	GetDocumentTextDetectionResults(startJobId);
                	break;
                }else{
            ...
            case ANALYSIS:
                StartDocumentAnalysis(strBucket, strDocument);
                if(isNullOrEmpty(startJobId)) {
                	GetDocumentAnalysisResults(startJobId);
                	break;
                }else{
                	System.out.println("analysis failed : JobId is null");
                	throw new Exception("JobId Exception");
    ...
    private static void StartDocumentTextDetection(String strBucket, String strDocument) throws Exception{
  ...
  //Gets the results of processing started by StartDocumentTextDetection
    private static void GetDocumentTextDetectionResults(String startJobId) throws Exception{
        int maxResults=1000;
    ...
    private static void StartDocumentAnalysis(String strBucket, String strDocument) throws Exception{
    ...
    private static GetDocumentAnalysisResult GetDocumentAnalysisResults(String startJobId) throws Exception{
        ...
        while (finished==false)
        {
            ...
            DocumentMetadata documentMetaData = docResponse.getDocumentMetadata();

            System.out.println("Pages: " + documentMetaData.getPages().toString());

            //Show blocks, confidence and detection times
            List<Block> blocks= docResponse.getBlocks();
            for (Block block : blocks) {
                DisplayBlockInfo(block);
            }
